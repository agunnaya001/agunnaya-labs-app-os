    ...
    <div className="space-y-4">
      ...
      {nfts.length === 0 ? (
        <div className="glass p-8 rounded-lg text-center">
          <p className="text-muted-foreground">No NFTs yet. Win battles in the arena to earn champion NFTs!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {nfts.map((nft) => (
            ...
            >
              ...
              <div className="p-4">
                <div className="mb-3">
                  ...
                  <Badge className={`text-xs ${getRarityColor(nft.rarity)}`}>{nft.rarity?.toUpperCase() || 'COMMON'}</Badge>
                </div>
                ...
                <Button size="sm" className="w-full bg-neon-green text-black font-bold hover:shadow-[0_0_20px_rgba(0,255,157,0.6)]">View Details</Button>
              </div>
            </div>
          ))}
        </div>
